...
public class MainSettingScreen extends Screen {
    ...
    public void render(@NotNull GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        */
        if (MouseOverRectLeft(mouseX, mouseY) || MouseOverRectRight(mouseX, mouseY)){
            Minecraft mc = Minecraft.getInstance();
            String settingScreenAlphaTipText = Component.translatable("cabbagejellypack.settingscreenalpha.tip").getString();
            // 获取文本宽度
            int textWidth = mc.font.width(settingScreenAlphaTipText);
            guiGraphics.pose().pushPose();
            guiGraphics.pose().translate(0, 0, 500);
            guiGraphics.fill(
                    mouseX - 2 - 1,
                    mouseY - mc.font.lineHeight - 2 - 1,
                    mouseX + textWidth + 2 + 1,
                    mouseY + 2 + 1,
                    0xFF000000
            );
            ...
            guiGraphics.fill(
                    mouseX - 2,
                    mouseY - mc.font.lineHeight - 2,
                    mouseX + textWidth + 2,
                    mouseY + 2,
                    0xFF333333
            );
            guiGraphics.drawString(
                    mc.font, settingScreenAlphaTipText,
                    mouseX, mouseY - mc.font.lineHeight,
                    0xFFFFFFFF,false
            );
            guiGraphics.pose().popPose();
        }
    ...
    // 手动检测矩形悬停
    private static boolean MouseOverRectLeft(int mouseX, int mouseY) {
        Minecraft mc = Minecraft.getInstance();
        int screenWidth = mc.getWindow().getGuiScaledWidth();
        int screenHeight = mc.getWindow().getGuiScaledHeight();
        return mouseX >= screenWidth / 2 - 88
                && mouseX <= screenWidth / 2 - 88 + 120
                && mouseY >= screenHeight / 2
                && mouseY <= screenHeight / 2 + 20;
    }

    private static boolean MouseOverRectRight(int mouseX, int mouseY) {
        Minecraft mc = Minecraft.getInstance();
        int screenWidth = mc.getWindow().getGuiScaledWidth();
        int screenHeight = mc.getWindow().getGuiScaledHeight();
        return mouseX >= screenWidth / 2 + 55
                && mouseX <= screenWidth / 2 + 55 + 33
                && mouseY >= screenHeight / 2
                && mouseY <= screenHeight / 2 + 20;
    }
