...
import com.FuFu.CabbageJellyPack.mixin.InventoryAccessor;
    ...
    public static void render(GuiGraphics guiGraphics) {
        ...
        if (clientLevel != null && localPlayer != null) {
            if (clientLevel.isClientSide) {
                ...
                int count = 0;
                // 首先将 Inventory 转换为访问器接口
                InventoryAccessor inventoryAccessor = (InventoryAccessor) localPlayer.getInventory();

                // 然后获取 items 列表
                NonNullList<ItemStack> items = inventoryAccessor.getItems();
